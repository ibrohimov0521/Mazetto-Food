...
try {
  ...
  assert.match(
    await robot.text(),
    /Sitemap: https:\/\/mazettofood.uz\/sitemap.xml/,
  );
  ...
  assert(
    paths.includes("/") &&
      paths.includes("/menu") &&
      paths.some((path) => path.startsWith("/product/")),
  );
  assert(!paths.some((path) => /cart|checkout|profile|orders/.test(path)));
  for (const path of [
    "/",
    "/menu",
    paths.find((path) => path.startsWith("/product/")),
  ]) {
    ...
    if (path.startsWith("/product/")) {
      ...
      assert(
        (await page.locator('a[href^="/product/"]').count()) > 5,
        "Products in initial HTML without JavaScript",
      );
    }
  }
  for (const path of [
    ...
    assert.match(
      (
        await page
          ...
          .evaluateAll((nodes) => nodes.map((node) => node.content))
      ).join(","),
      /noindex/,
    );
  }
  const missing = await ctx.request.get(
    web + "/product/qa-product-does-not-exist",
  );
  assert.equal(missing.status(), 404);
  checks.push(
    "Sitemap/robots, canonical metadata, private noindex, product JSON-LD, missing product 404, real product HTML without JavaScript",
  );
  await ctx.close();
  ...
  if (new URL(web).hostname === "127.0.0.1") {
    await client.route("**/api/v1/**", async (route) =>
  ...
  let ticket = {
    ...
    order: {
      ...
      type: "DELIVERY",
      branch: { name: "QA" },
      items: [{ id: "qa-item", productName: "Lavash", quantity: "1" }],
    },
  };
  ...
  await kctx.route("**/api/v1/**", async (route) => {
    ...
    if (path.endsWith("/kitchen/orders"))
      return readFails
        ? json("QA network interruption", 503)
        : json({ items: ticket ? [ticket] : [], hasMore: false, limit: 250 });
    if (path.includes("/kitchen/orders/")) {
      actionCount++;
      if (rejectAction) {
        ticket = null;
        return json("Buyurtma bekor qilingan. Holat yangilandi.", 400);
      }
  ...
  assert.equal(
    await kitchen
      ...
      .count(),
    0,
    "Successful action survives failed refresh",
  );
  readFails = false;
  rejectAction = true;
  await kitchen
    ...
    .click();
  await kitchen
    .getByText("Buyurtma bekor qilingan. Holat yangilandi.")
    .waitFor();
  await kitchen.locator("article").waitFor({ state: "hidden" });
  await kitchen.screenshot({ path: output + "/kitchen-recovered.png" });
  checks.push(
    "Kitchen action updates immediately, refresh failures preserve correct status, stale cancelled ticket disappears while action error stays visible",
  );
  await kctx.close();
