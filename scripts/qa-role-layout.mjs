...
try {
  for (const viewport of [
    ...
    const metrics = await page.evaluate(() => {
      ...
      const firstTicket = document.querySelector('section[aria-label="Yangi"] article');
      const firstAction = firstTicket?.querySelector("button[title='Qabul qilish']");
      ...
      return {
        ...
        logoVisible: getComputedStyle(document.querySelector(".mz-panel-logo")).display !== "none",
        brandVisible: getComputedStyle(document.querySelector(".mz-panel-brand")).display !== "none",
      };
    });
    ...
    assert.ok(Math.abs(metrics.shellRight - viewport.width) <= 2, "Shell does not fill viewport");
    assert.ok(Math.abs(metrics.headerRight - viewport.width) <= 2, "Header does not fill viewport");
    ...
    if (viewport.width >= 1200) {
      const cardPositions = await page
        ...
        .evaluateAll((cards) => cards.slice(0, 2).map((card) => card.getBoundingClientRect().left));
      if (viewport.width >= 1500) {
        assert.notEqual(cardPositions[0], cardPositions[1], "Orders are not in two columns");
      } else {
        assert.equal(cardPositions[0], cardPositions[1], "Orders are too narrow");
      }
    }
    ...
    if (viewport.width === 1600) {
      ...
      assert.equal(
        await page.getByRole("complementary", { name: "Ish joylari menyusi" }).isVisible(),
        false,
      );
    ...
    if (viewport.width < 768) {
      ...
      await page.getByRole("complementary", { name: "Ish joylari menyusi" }).waitFor();
      await page.locator("#staff-panel-menu").getByRole("button", { name: "Menyuni yopish" }).click();
      const card = page.locator('section[aria-label="Yangi"] article').first();
      await card.getByRole("button", { name: /buyurtmani qisqartirish/ }).click();
      assert.equal(await card.locator("li").count(), 0, "Compact card still shows items");
    }
    ...
    for (const viewport of [{ width: 1280, height: 800 }, { width: 360, height: 780 }]) {
      ...
      await page.route("**/api/v1/**", (request) =>
        request.fulfill({
          status: 503,
          contentType: "application/json",
          body: JSON.stringify({ success: false, error: { message: "QA API unavailable" } }),
        }),
      );
      await page.goto(`${baseUrl}${route.path}`, { waitUntil: "domcontentloaded" });
      await page.getByRole("heading", { name: route.title, exact: true }).first().waitFor();
      const metrics = await page.evaluate(() => {
        const shell = document.querySelector("main");
        return {
          viewport: window.innerWidth,
      ...
      assert.ok(metrics.scrollWidth <= viewport.width + 1, `${route.path} horizontal overflow`);
      assert.ok(Math.abs(metrics.shellRight - viewport.width) <= 2, `${route.path} shell width`);
      assert.ok(Math.abs(metrics.headerRight - viewport.width) <= 2, `${route.path} header width`);
      assert.equal(metrics.brandVisible, viewport.width < 768);
      process.stdout.write(`${route.path} ${viewport.width} ${JSON.stringify(metrics)}\n`);
      await page.close();
    }
  }
} finally {
