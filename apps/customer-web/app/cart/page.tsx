...
function Checkout() {
  ...
  async function submitOrder() {
    if (!customer?.accessToken) {
      setMessage("Buyurtma berish uchun avval bosh sahifada telefon raqamingizni tasdiqlang.");
      return;
    }
    ...
    try {
      ...
      clearCart();
      setMessage(`${result.order?.orderNumber ?? result.customerOrder.id} raqamli buyurtma oshxonaga yuborildi.`);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-6 px-4 pb-28 pt-6 lg:grid-cols-[1fr_420px] lg:pb-6">
      <div className="grid gap-4">
        <div className="rounded-xl bg-white p-5 shadow-[0_16px_55px_rgba(15,118,110,0.12)]">
          <div className="flex items-end justify-between gap-3">
            <div>
              <p className="text-sm font-black uppercase text-emerald-700">Rasmiylashtirish</p>
              <h1 className="mt-1 text-3xl font-black text-neutral-950">Savatingiz</h1>
            </div>
            <span className="rounded-xl bg-emerald-50 px-4 py-2 text-sm font-black text-emerald-700">{items.length} ta mahsulot</span>
          </div>
          {items.length ? (
                  ...
                  <div>
                    <div className="flex justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-neutral-500">{item.variantName ?? "Oddiy"}</p>
                      </div>
                      <button className="pressable text-sm font-bold text-red-600" onClick={() => removeItem(item.key)} type="button">O'chirish</button>
                    </div>
                    ...
                    {item.notes ? <p className="mt-1 text-xs font-semibold text-neutral-500">Izoh: {item.notes}</p> : null}
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button className="pressable h-9 w-9 rounded-full bg-neutral-100 font-bold" onClick={() => updateQuantity(item.key, item.quantity - 1)} type="button">-</button>
            ...
            <div className="mt-5 rounded-xl bg-emerald-50 p-8 text-center">
              <p className="font-bold text-emerald-800">Savatingiz hozircha bo'sh.</p>
              <Link className="pressable mt-4 inline-flex rounded-2xl bg-emerald-600 px-5 py-3 font-bold text-white" href="/menu">Menyuni ochish</Link>
            </div>
          )}
        </div>
      </div>
      ...
      <aside className="h-fit rounded-xl bg-white p-5 shadow-[0_16px_55px_rgba(15,118,110,0.12)]">
        <h2 className="text-2xl font-black text-neutral-950">Buyurtmani rasmiylashtirish</h2>
        <div className="mt-4 grid gap-3">
          <input className="rounded-2xl border border-neutral-200 px-4 py-3 outline-none focus:border-emerald-500" placeholder="Ismingiz" value={name} onChange={(event) => setName(event.target.value)} />
          <input className="rounded-2xl border border-neutral-200 px-4 py-3 outline-none focus:border-emerald-500" placeholder="Telefon raqam" value={phone} onChange={(event) => setPhone(event.target.value)} />
          ...
          {type === "DELIVERY" ? (
            <textarea className="min-h-24 rounded-2xl border border-neutral-200 px-4 py-3 outline-none focus:border-emerald-500" placeholder="Yetkazib berish manzili" value={address} onChange={(event) => setAddress(event.target.value)} />
          ) : null}
        ...
        <div className="mt-5 grid gap-3 rounded-xl bg-emerald-50 p-4">
          ...
          <div className="flex justify-between text-sm font-bold text-neutral-600">
            ...
            <span>{deliveryFee ? formatMoney(deliveryFee) : "Bepul"}</span>
          </div>
        ...
        <button className="pressable mt-4 w-full rounded-2xl bg-emerald-600 px-5 py-4 font-black text-white disabled:opacity-50" disabled={submitting || !items.length || !branchId || !customer?.accessToken || !name || !phone || (type === "DELIVERY" && !address)} onClick={() => void submitOrder()} type="button">
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-emerald-100 bg-white/95 p-3 shadow-[0_-12px_30px_rgba(17,24,39,0.10)] backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase text-neutral-500">Jami</p>
            <p className="text-lg font-black text-neutral-950">{formatMoney(total)}</p>
          </div>
          <button className="pressable rounded-xl bg-[#16A34A] px-5 py-4 font-black text-white disabled:opacity-50" disabled={submitting || !items.length || !branchId || !customer?.accessToken || !name || !phone || (type === "DELIVERY" && !address)} onClick={() => void submitOrder()} type="button">
...
function choiceClass(active: boolean): string {
  return `pressable rounded-xl px-4 py-3 text-sm font-bold ${active ? "bg-[#16A34A] text-white" : "bg-emerald-50 text-emerald-800"}`;
}
